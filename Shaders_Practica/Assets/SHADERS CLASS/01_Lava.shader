    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                o.noise_uv = TRANSFORM_TEX(v.uv, _NoiseTexture);
                o.noise_uv += _Flow2.xy * _Time.x;
    
                //EXTRA FLOW MAP
                float2 flowVector = tex2Dlod(_NoiseTexture, float4(o.uv, 0, 0)).rg;
            ...
            {               
                float2 uv = i.uv;
                fixed4 noise = tex2D(_NoiseTexture, i.noise_uv);
                fixed2 disturb = noise.xy * 0.5 - 0.5;
                fixed4 col = tex2D(_MainTex, i.uv + disturb);
                fixed noisePulse = tex2D(_NoiseTexture, i.noise_uv + disturb).a;
